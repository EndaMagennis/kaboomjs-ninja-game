...
function attack(){
    ...
    if(player.curAnim() !== playerAttackAnim && player.isDead === false){
        ...
        player.play(playerAttackAnim, {onEnd: ()=>{
            ...
            //play is a kaboom function wich plays an audio asset
...
scene("MainGame", () =>{
     ...
     * By default the camera is centered. */
    camScale(1.2);

    /**
     * setGravity sets a virtual gravity which acts on objcets with a .body() attribute.
     * Objects with a .body() attribute will respond to gravity unless stated as static.
     */
    setGravity(1000); 
    ...
    /**
     * onUpdate() is a kaboom function which runs each frame of the game.
     * It can be used to update logical checks.
     */
    onUpdate(( )=>{
 ...
 * Can also pass an args parameter in order to, for example, reset or reinitialise the scene.
 */
go("MainMenu");